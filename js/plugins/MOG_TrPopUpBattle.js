 ...
 * @author Moghunter （Drill_up翻译） 移植MZ:飞天大胖喵
 * @target MZ
 * @param 是否死亡后立即掉落
 * @type boolean
 * @on 立即掉落
...
Sprite_Enemy.prototype.updateCollapse = function () {
    ...
    if (!this._enemy._treasure.checked) { this.checkTreasurePopup() };
    // if (this._effectDuration === 0 && !this._enemy._treasure.checked) {console.log('01 - MOG - 敌人倒下了');this.checkTreasurePopup()};
};
...
/*
//MOG处理过程:
Game_Enemy.prototype.initMembers() //初始化成员
    this._treasure = {};				//添加敌人掉落物存储对象
    this._treasure.needPopup = false;	//是否需要弹出掉落物品图标
    this._treasure.checked = false;		//是否检查过敌人掉落物
    this._treasure.item = [];			//初始化敌人掉落物数组
Sprite_Enemy.prototype.updateCollapse 	   //当敌人倒下
Sprite_Enemy.prototype.updateBossCollapse  		//当BOSS倒下
Sprite_Enemy.prototype.updateInstantCollapse    //当瞬间倒下 ↓ 去生成掉落物
Game_Enemy.prototype.makeDropItems() // 生成掉落物品

    if (this._treasure.checked) {  //检查是否生成过掉落物品，如果是就返回掉落的物品，否则就生成
        return this._treasure.item;
    }else{ //去生成 }

Game_Enemy._treasure.item //保存掉落物品的对象
